...
#### 20250317 sql 5번째 수업!!!!!!! 총 30번째 수업!!!!!!! 헉!!!!!
## 5일차
- VIEW : [뷰쿼리](./day05/sql01_뷰.sql)
    - 기존 테이블에서 **권한 별**로 보일 수 있는 컬럼을 지정해서 만드는 개체(보안 목적)
    - 기존 테이블 중 개인정보나 중요한 부분이 있으면 제외하고 보일 수 있음
    - 뷰(테이블)라도 INSERT, UPDATE, DELETE가 가능함. (단일뷰에서만 !!)
       ```sql
       CREATE VIEW 뷰 이름
       AS
            SELECT 쿼리;
        [WITH READ ONLY]
       ```
    - 복합뷰는 두 개 이상의 테이블을 JOIN해서 만든 뷰. *DML 기능이 불가* !!

- 서브쿼리 : [서브쿼리](./day05/sql02_서브쿼리.sql)
    - 메인 쿼리를 도와주는 하위 쿼리를 뜻함. 소괄호() 내에 포함됨
    - 단일행 서브쿼리, 다중행 서브쿼리마다 사용법이 다름
    - SELECT절(스칼라) 서브쿼리, FROM절 서브쿼리 *WHERE절 서브쿼리*(이게 제일 쉽대!!)
...
- 시퀀스 : [시퀀스쿼리](./day05/sql03_시퀀스.sql)
    ...
    - **Oracle만 존재 !!**
        ...
        INCREMENT BY 1       -- 증가값
        START WITH 1         -- 초기 시작값
        [MAXVALUE 9999999]   -- 최대 증가값
        [CYCLE]              -- 최대 증가값에 도달하면 다시 처음 1로 돌아갈 것인지
        [CACHE]              -- 번호 증가 캐쉬(대용량 삽입시만 관계)

        시퀀스명.nextval  : 번호 넘기기
        시퀀스명.currval  : 현재 번호 확인하기
        ```

- 사용자 계정 권한 : [쿼리](./day05/sql04_사용자계정관리.sql)
    <img src="./image/db005.png" width="700">
    > 함부로 권한을 주지 않음으로써 system 내의 database를 관리 및 보호할 수 있다 !!
    - 사용자 생성 후 권한(ROLE)을 부여해야 스키마(SCHEMA)를 사용할 수 있다!
        ```sql
        -- 권한 부여
...
#### 20250318 sql 6번째 수업!!!!! 오늘 31번째 수업이야!!! 베스킨라빈스 31!!!
## 6일차 
...
- Stored Procedure와 Function을 만들기 위해서 사용
    - 저장 프로시저
        - 한꺼번에 많은 일을 수행해야 할 때(Transaction당 수해되는 로직들을 묶어서)
         > 예) 한 번에 5개의 테이블에서 조회와 DML를 처리해야 한다
         >   - 쿼리를 최소 10개를 수행해야 함
    ...
    - 함수
        ...
        SELECT *, 함수명(파라미터)
          FROM 컬럼명;
        ```
    - 커서 ; 데이터베이스에서 반드시 필요한 것!!
        - DB에서 테이블에 들어 있는 테이터를 한 줄씩 읽기 위해서 필요
    
    - 트리거
        - 특정 동작으로 다른 테이블에 자동으로 데이터가 변경되는 기능
        - 한 가지 동작에 대해서 연쇄적으로 다른 일 발생
        ```sql
